...
data = data.tolist()
data_sample = [0*np.pi/180.0, -50.462*np.pi/180.0, -9.704*np.pi/180.0, 0*np.pi/180.0, 62.899*np.pi/180.0, 0*np.pi/180.0]
data_go = [-84.356*np.pi/180.0, -50.462*np.pi/180.0, -9.704*np.pi/180.0, 0*np.pi/180.0, 62.899*np.pi/180.0, 0*np.pi/180.0]
print(data_sample)
...
if clientID!=-1:
    ...
    res, TipHandle = vrep.simxGetObjectHandle(clientID, 'Epson_ikTip#', vrep.simx_opmode_oneshot_wait)
    ...
    for i in range(1):
        ...
        if (res==0) and len(path)>0:
            ...
            f = open('path.txt','w')
            while runningPath:
                res, retInts,position,posmatrix,retBuffer=vrep.simxCallScriptFunction(clientID,'remoteApiCommandServer',vrep.sim_scripttype_childscript,'getObjectPose',[TipHandle],[],[],emptyBuff,vrep.simx_opmode_oneshot_wait)
                res, retInts, orientation, posmatrix, retBuffer = vrep.simxCallScriptFunction(clientID,
                ...
                position = listToStringWithoutBrackets(position)
                orientation = [i * 180/3.14159 for i in orientation]
                orientation = listToStringWithoutBrackets(orientation)
                f.write(position+',')
                f.write(orientation+'\n')
